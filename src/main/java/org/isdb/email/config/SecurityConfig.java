	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http, JwtTokenProvider jwtTokenProvider)
		...
		http.csrf(AbstractHttpConfigurer::disable) // Disable CSRF for REST APIs
				.authorizeHttpRequests(auth -> auth.requestMatchers("/api/auth/login").permitAll()
						.requestMatchers("/user").permitAll().requestMatchers("/api/public/**").permitAll()
...
////                .roles("REGULAR_USER")
//				.authorities("ROLE_REGULAR_USER", "SENIOR").build();
//
//		System.out.println(regularUser.getAuthorities());
//
//		UserDetails hemel = User.builder().username("hemel").password(passwordEncoder().encode("hemel123"))
////                .roles("REGULAR_USER")
//				.authorities("ROLE_REGULAR_USER", "JUNIOR").build();
//
...
//		UserDetails manager = User.builder().username("manager").password(manager123).roles("MANAGER").build();
//
